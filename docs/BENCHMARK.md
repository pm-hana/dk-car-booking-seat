...
| B-4 상태 3단계화 | **Nguyễn Hoàng Qúy** / 리포트 재구조화 | *"There are only two states (red/not red)"* → PASS/WARNING/FAIL 3단계 | 배치도 좌석을 승인 완료(초록 실선) / 승인 대기(주황 **파선**) / 빈자리(파랑) 3단계로. 색 조합은 검증 도구로 선정 — 초록·주황·빨강 3색안은 주황↔빨강 ΔE 10.8로 폐기, 채택안도 적록 색각 경계(6.6)라 규칙대로 파선·범례·툴팁 문구를 보조 인코딩으로 동반 | **적용** | 0807 ver.2 |
